...
/// Sentinel used for optional values encoded as a JS `number`.
///
/// `2^53 - 1` (`Number.MAX_SAFE_INTEGER`) is exactly representable in `f64`,
/// remains outside the 32-bit integer range, and cannot arise from an `f32`
/// round-trip. Reserving it for pointer-sized values still leaves about 9 PB
/// of address space on wasm64.
