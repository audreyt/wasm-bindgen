...
use crate::js::{
    define_raw_wasm_ptr_slot, raw_wasm_ptr_expr, raw_wasm_ptr_slot, strip_raw_wasm_ptr_expr,
    Context,
};
            ...
            match constructor {
                Some(name) if js.cx.resolve_class_name(name) == resolved_class => {
                    ...
                    let (ptr_assignment, register_data) = if js.cx.memory64 {
                        js.cx.expose_raw_wasm_ptr_symbol();
                        let raw_ptr_assignment = define_raw_wasm_ptr_slot("this", &raw);
