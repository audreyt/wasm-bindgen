...
use crate::js::{raw_wasm_ptr_expr, strip_raw_wasm_ptr_expr, Context};
...
impl<'a, 'b> JsBuilder<'a, 'b> {
    ...
    /// Coerce a JS pointer-sized value into the exact Wasm pointer ABI.
    ...
    pub fn class_raw_ptr(&self, expr: &str) -> String {
        if self.cx.memory64 {
            format!("{expr}.__wbg_ptr_raw")
        } else {
    ...
    fn assert_number(&mut self, arg: &str) {
        if self.cx.memory64 {
            if let Some(arg) = strip_raw_wasm_ptr_expr(arg) {
...
fn instruction(
    ...
    match instr {
        ...
        Instruction::I32FromExternrefRustOwned { class } => {
            ...
            js.push(js.coerce_class_ptr_arg(&format!("ptr{i}")));
        }
        ...
        Instruction::I32FromExternrefRustBorrow { class } => {
            ...
            js.assert_class(&val, class);
            js.assert_not_moved(&val);
            let raw_ptr = js.class_raw_ptr(&val);
            js.push(js.coerce_class_ptr_arg(&raw_ptr));
        }
        ...
        Instruction::I32FromOptionRust { class } => {
            ...
            js.cx.expose_is_like_none();
            let i = js.tmp();
            let zero = js.size_literal(0);
            js.prelude(&format!("let ptr{i} = {zero};"));
            ...
            js.push(format!(
                "ptr{i} ? {} : {zero}",
                js.coerce_class_ptr_arg(&format!("ptr{i}")),
            ));
        }
        ...
        Instruction::RustFromI32 { class } => {
            ...
            let js_ptr = js.cx.to_js_ptr_inline(&val);
            let raw = js.coerce_raw_ptr(&val);
            match constructor {
                Some(name) if js.cx.resolve_class_name(name) == resolved_class => {
                    ...
                    let (ptr_assignment, register_data) = if js.cx.memory64 {
                        if js.cx.config.generate_reset_state {
                            (
                                format!(
                                    "\
                                    this.__wbg_ptr = {js_ptr};
                                    Object.defineProperty(this, '__wbg_ptr_raw', {{ value: {raw}, writable: true }});
                                    this.__wbg_inst = __wbg_instance_id;
                                    "
                                ),
                                format!("{{ ptr: {raw}, instance: __wbg_instance_id }}"),
                            )
                        } else {
                            (
                                format!(
                                ...
                                "this.__wbg_ptr_raw".to_string(),
                            )
                        }
                    } else if js.cx.config.generate_reset_state {
                        (
                            format!(
                                "\
                                this.__wbg_ptr = {js_ptr};
                            ...
                            format!("{{ ptr: {js_ptr}, instance: __wbg_instance_id }}"),
                        )
                    } else {
                        (
                            format!("this.__wbg_ptr = {js_ptr};"),
        ...
        Instruction::View { kind, mem } => {
            let len = js.pop();
            let ptr = js.pop();
            // For externref types, use the view version that doesn't take ownership
            let f = match kind {
                VectorKind::Externref | VectorKind::NamedExternref(_) => {
                    js.cx.expose_get_array_js_value_view_from_wasm(*mem)
                }
